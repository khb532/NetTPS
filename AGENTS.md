...
## 에이전트 지침
 ...
 - 작업 결과 보고 시 Codex는 삭제된 문구는 `<span style="color:red">...</span>` 형태로 빨간색 텍스트로, 수정·추가된 문구는 `<span style="color:green">...</span>` 형태로 초록색 텍스트로 표시하여 사용자에게 보고합니다. 해당 색상 표기 없이 결과를 보고하지 않습니다.
